...
Scrapes video list from /video/search, categories, etc.
...
class PornHubCrawler(BaseCrawler):
    ...
    async def _parse_video_page(self, video_url: str) -> VideoInfo | None:
        try:
            ...
            # Viewkey
            vk_m = re.search(r'viewkey=([a-z0-9]+)', video_url)
            viewkey = vk_m.group(1) if vk_m else ""

            # Video ID
            vid_m = re.search(r'data-video-id="(\d+)"', html)
            video_id = vid_m.group(1) if vid_m else viewkey

            # Cover (from data-poster, not data-mediabook)
            cover_urls = []
    ...
    async def _scrape_list_page(self, url: str) -> list[str]:
        try:
            ...
            urls = re.findall(
                r'href="(/view_video\.php\?viewkey=[a-z0-9]+)"', html
            )
    ...
    async def get_latest_videos(self) -> AsyncGenerator[VideoInfo, None]:
        async for vu in self._paginate("https://www.pornhub.com/"):
