...
Scrapes video list from /most_viewed/, etc.
...
class ThumbzillaCrawler(BaseCrawler):
    @property
    def name(self) -> str:
        return "thumbzilla"
    ...
    async def _parse_video_page(self, video_url: str) -> VideoInfo | None:
        try:
            ...
            return VideoInfo(
                source=self.name,
                video_id=video_id,
                title=title or f"thumbzilla_{video_id}",
                page_url=video_url,
    ...
    async def get_latest_videos(self) -> AsyncGenerator[VideoInfo, None]:
        async for vu in self._paginate("https://www.thumbzilla.com/most_viewed/"):
